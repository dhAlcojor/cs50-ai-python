...
def generate_solution(node):
    """
    Returns the solution to the problem given the final node.
    The solution is an array of (movie_id, person_id) pairs
    """
    """Array with (movie_id, person_id) pairs that connect the source to the target."""
    cells = []
    while node.parent is not None and node.state[0] is not None:
    ...
    while True:
        ...
        for state in neighbors_for_person(node.state[1]):
            ...
            if (child.state[1] == target):
                return generate_solution(child)
